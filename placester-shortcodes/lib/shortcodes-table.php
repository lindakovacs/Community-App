...
<?php
	...
	public function single_row( $post ) {
		...
		if ($sc_options!==false && !empty($sc_options['width']) && !empty($sc_options['height'])) {
			$embed_str = htmlentities('<script id="plwidget-'.$post->ID.'" src="'.PLACESTER_PLUGIN_URL.'placester-shortcodes/fetch-widget.js?id='.$post->ID.'" style="width:'.$sc_options['width'].'px;height:'.$sc_options['height'].'px"></script>');
